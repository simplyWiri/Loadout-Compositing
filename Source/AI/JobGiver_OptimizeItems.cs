    ...
    public class JobGiver_OptimizeItems : ThinkNode_JobGiver {
        ...
        public override Job TryGiveJob(Pawn pawn) {
            ...
            var items = pawn.inventory.innerContainer.InnerListForReading
                .ConcatIfNotNull(pawn.equipment.AllEquipmentListForReading).ToList();
            ...
            foreach (var item in requiredItems) {
                ...
                foreach (var thing in thingsOnMap) {
                    if (!pawn.CanReserve(thing) || !pawn.CanReach(thing, PathEndMode.Touch, Danger.Unspecified) ||
                        thing.IsForbidden(pawn)) continue;

                    var job = JobMaker.MakeJob(JobDefOf.TakeInventory, thing);
                    if (thing.def.category == ThingCategory.Item) {
                        var currentlyHeldAmount = items.Where(th => th.def == item.Def && item.Filter.Allows(th))
                            .Sum(t => t.stackCount);
                        job.count = Mathf.Min(item.Quantity - currentlyHeldAmount,
                            MassUtility.CountToPickUpUntilOverEncumbered(pawn, thing));
                    }

                    job.checkEncumbrance = true;
                    if (MassUtility.WillBeOverEncumberedAfterPickingUp(pawn, thing, job.count)) {
                        continue;
                    }
